...
public abstract class BaseNetListener<T> implements Subscriber<T> {
    ...
    public BaseNetListener(BaseHttpListener baseHttpListener, String str) {
        this.baseHttpListener = baseHttpListener;
        if (this.baseHttpListener != null ) {
            this.baseHttpListener.showWaitDialog(str);
        }
    }
    @Override
    ...
    public void onError(Throwable e) {
        ...
        if (e instanceof HttpException) {
            ...
            if (resultException.getErrCode() == -999) {//token失效 需要重新登录
                if (this.baseHttpListener != null) {
                    this.baseHttpListener.restartLogin();
                }
            } else if (resultException.getErrCode() == -997)//token为空  很大肯定是应用被回收了 不显示错误消息
            {
                return;
            }
